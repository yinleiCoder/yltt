...
export default function AdminPhotosPage() {
  ...
  const handlePageChange = (newPage) => {
    if (newPage === page || newPage < 1 || newPage > totalPages || pageLoading)
      return;
    setPage(newPage);
  ...
  // Reset GSAP when page changes (page loading triggers opacity)
  useGSAP(
    () => {
      if (pageLoading || !loaded || !photos?.length) return;
      gsap.fromTo(
        ".photo-card",
        { scale: 0.94, opacity: 0, y: 12 },
        {
    ...
    { scope: gridRef, dependencies: [pageLoading] },
  );
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            共 {totalCount} 张照片（双击图片进行预览）
          </p>
        </div>
        <Button
          onClick={() => setDialogOpen(true)}
          className="bg-primary hover:bg-primary/90 text-primary-foreground text-xs h-8 gap-1.5"
        >
          <Plus size={14} />
          上传照片
        </Button>
      ...
      {!loaded ? (
        <div className="columns-2 md:columns-3 lg:columns-5 gap-3">
          {Array.from({ length: 10 }).map((_, i) => (
            <div
              key={i}
              className="surface-card overflow-hidden mb-3 break-inside-avoid"
            >
              <Skeleton
                className="w-full"
                style={{ aspectRatio: `${[0.75, 0.8, 0.667, 1, 0.9][i % 5]}` }}
              />
              <div className="p-2.5">
        ...
        <div className="text-center py-20">
          ...
          <p className="text-xs text-muted-foreground">还没有照片</p>
        </div>
      ) : (
        <>
          <div
            ref={gridRef}
            className={`columns-2 md:columns-3 lg:columns-5 gap-3 transition-opacity duration-200 ${pageLoading ? "opacity-50 pointer-events-none" : ""}`}
          >
            {photos?.map((p) => (
              <Card
                key={p.id}
              ...
              >
                ...
                >
                  {p.url ? (
                    ...
                    <div className="aspect-square w-full bg-accent flex items-center justify-center">
                ...
                {p.title ? (
                  ...
                  >
                    <p className="text-[10px] text-muted-foreground truncate hover:text-primary transition-colors">
                      {p.title}
                    </p>
                  </CardContent>
                ) : (
            ...
            {!running && !allDone && (
              <label className="flex items-center justify-center w-full h-28 border-2 border-dashed border-border rounded-lg cursor-pointer hover:border-primary/40 transition-colors">
                <div className="text-center">
                  <Upload
                    size={22}
                    className="mx-auto text-muted-foreground mb-1.5"
                  />
            ...
            {totalCountQ > 0 && (
              <div className="space-y-2">
                ...
                {totalCountQ > 0 && !allDone && (
                  <Progress
                    value={((doneCount + errorCount) / totalCountQ) * 100}
                    className="h-1"
                  />
                )}
                  ...
                  {queue.map((q) => (
                    ...
                    >
                      ...
                      {q.status === "error" && q.error && (
                        <span
                          className="text-[10px] text-red-500/70 shrink-0 max-w-[140px] truncate"
                          title={q.error}
                        >
                          {q.error}
                        </span>
                  ...
                  {totalCountQ > 0 && (
                    <>
                      ...
                      >
                        ...
                        {running
                          ? "上传中..."
                          : `开始上传 (${queue.filter((q) => q.status === "pending").length})`}
                      </Button>
                    </>
                  )}
                  {totalCountQ === 0 && (
      ...
      <Dialog open={editDialogOpen} onOpenChange={setEditDialogOpen}>
        <DialogContent className="max-w-sm bg-card border-border">
          ...
          <div className="space-y-3 mt-2">
            {editingPhoto?.url && (
              <img
                src={getFileUrl(editingPhoto.url)}
                alt=""
            ...
            <Button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  setPhotos((prev) =>
                    prev.map((p) =>
                      p.id === editingPhoto.id ? { ...p, title: editTitle } : p,
                    ),
                  );
                  setEditDialogOpen(false);
                } catch (err) {
      ...
      <Dialog open={!!previewPhoto} onOpenChange={() => setPreviewPhoto(null)}>
        <DialogContent
          className="max-w-3xl bg-card border-border p-1"
            ...
            <div className="flex flex-col gap-2">
              <div className="flex items-center justify-between px-3 pt-0">
                <DialogTitle className="text-sm font-medium truncate">
                  {previewPhoto.title || "照片预览"}
                </DialogTitle>
              </div>
              <img
                src={getFileUrl(previewPhoto.url)}
                alt={previewPhoto.title || ""}
                className="w-full max-h-[75vh] object-contain rounded-b-lg"
                loading="lazy"
              />
            </div>
          )}
