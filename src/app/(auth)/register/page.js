...
export default function RegisterPage() {
  ...
  useGSAP(() => {
    gsap.set('.register-field', { y: 20, opacity: 0 })
    gsap.set(cardRef.current, { opacity: 0, scale: 0.95 })
    gsap.to(cardRef.current, { opacity: 1, scale: 1, duration: 0.5, ease: 'power3.out' })
    gsap.to('.register-field', { y: 0, opacity: 1, duration: 0.4, stagger: 0.08, ease: 'power2.out', delay: 0.15 })
  }, [])
  ...
  const handleRegister = async (e) => {
    ...
    setTimeout(() => { router.push('/dashboard'); router.refresh() }, 300)
  }
    ...
    <Card ref={cardRef} className="border-border bg-card">
      ...
      <CardContent>
        <form onSubmit={handleRegister} className="space-y-4">
          {error && <div ref={errorRef} className="p-3 rounded-md bg-destructive/10 border border-destructive/20 text-destructive text-xs">{error}</div>}
          ...
          <Button type="submit" className="w-full bg-primary hover:bg-primary/90 text-primary-foreground register-field" disabled={loading}>
            {loading ? <Loader2 size={14} className="animate-spin mr-2" /> : null}注册
          </Button>
        </form>
        <Separator className="my-5 bg-border" />
        <p className="text-center text-xs text-muted-foreground register-field">
